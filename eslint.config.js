...
module.exports = defineConfig([
	...
	// ---------------------------------------------------------------------------
	// eslint-config-prettier — MUST be the LAST element of this array.
	//
	// It only ever turns rules OFF: every eslint rule whose job is layout, and
	// which therefore fights `prettier --write`. Two formatters with overlapping
	// jurisdiction make a repo unfixable — `lint --fix` and `format:fix` demand
	// opposite things and neither can be satisfied. Measured on the pilot
	// (ConductionNL/larpingapp#321): without this line, formatting the frontend
	// left 29 eslint errors, every one a layout rule disagreeing with prettier.
	//
	// It disables no CORRECTNESS rule. In particular all 21 `vue/no-deprecated-*`
	// rules that `conductionVue3Fixes` switches on above remain present and ON —
	// verified with `eslint --print-config`. `indent` moves from eslint to
	// prettier's `useTabs: true`, which is the same tab, from the tool that also
	// reaches CSS and SCSS.
	// ---------------------------------------------------------------------------
